...
public interface IDynamicObjectRepository
{
    ...
    Task<DynamicObject?> GetByIdAsync(Guid id, Guid tenantId, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<DynamicObject>> GetAllAsync(Guid tenantId, int pageNumber = 1, int pageSize = 50, CancellationToken cancellationToken = default);
    ...
    Task<DynamicObject> UpdateAsync(DynamicObject dynamicObject, Guid tenantId, CancellationToken cancellationToken = default);
    ...
    Task<bool> ExistsByNameAsync(string objectName, Guid tenantId, Guid? excludeId = null, CancellationToken cancellationToken = default);
}
